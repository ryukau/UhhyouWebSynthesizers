...
class NumberInput {
  constructor(
    ...
    this.range.addEventListener(
      "input", (event) => this.onInputRange(event), false)
    this.number.addEventListener(
      "change", (event) => this.onInputNumber(event), false)
  }

  addRangeInput() {
    this.range = this.addInput(
      "range", this.defaultValue, this.min, this.max, this.step)
...
class NumberInputLog extends NumberInput {
  ...
  onInputRange(event) {
    this.number.value = (2 ** (this.range.value * this.maxInLog) - 1)
      * this.valueRange / this.maxInLinear + this.min
    this.onInputFunc(this.number.value)
  }
}
