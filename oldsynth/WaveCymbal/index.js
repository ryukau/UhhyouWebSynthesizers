...
function makeWave() {
  ...
  for (var ch = 0; ch < channels; ++ch) {
    ...
    workers[ch].worker.postMessage({
      ...
      attenuation: transformAttenuation(inputAttenuation.value),
...
function transformAttenuation(x) {
  return 1 - Math.exp(Math.log(1e-4 / 0.2) * x + Math.log(0.2))
}
...
var inputAttenuation = new NumberInput(divCymbalControls.element,
  "Attenuation", 0.5, 0, 1, 0.0001, refresh)
