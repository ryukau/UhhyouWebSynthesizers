class WaveView extends Canvas {
  ...
  fixY(y) { return (this.isUpperHalf ? this.height : this.center.y) * y; }
...
class EnvelopeView extends Canvas {
  ...
  clampX(value) { return Math.max(0, Math.min(value, 1)); }
  ...
  set value(v) {
    this.setControlPoints(v.x1, v.y1, v.x2, v.y2);
    this.draw();
  }
  ...
  getMouseMove(event) { return new Vec2(event.movementX, event.movementY); }

  onLoad(event) { this.grabbed = null; }
...
class EnvelopeControl {
  constructor(
    ...
    this.envelopeView = new EnvelopeView(
      this.div.element, width, height, x1, y1, x2, y2, label, refreshFunc);

    this.divInput = new Div(this.div.element, label);
    this.inputAmount = new NumberInput(
      this.divInput.element, "Amount", amount, -1, 1, 0.01, refreshFunc, false);
    this.inputBias = new NumberInput(
      this.divInput.element, "Bias", bias, minBias, maxBias, stepBias, refreshFunc,
      isBiasLogKnob);
  }
