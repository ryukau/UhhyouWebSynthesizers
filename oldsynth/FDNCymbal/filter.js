...
class StateVariableFilter {
  ...
  set cutoff(value) {
    this.fc = 2 * Math.sin(Math.PI * value / this.sampleRate)
    console.log(this.fc)
  }
...
class SVFStack {
  constructor(sampleRate, stack = 4) {
    this.svf = []
    for (var i = 0; i < stack; ++i) {
      this.svf[i] = new StateVariableFilter(sampleRate)
    }
  }

  set cutoff(value) {
    for (let svf of this.svf) {
      svf.cutoff = value
    }
  }
...
function filterPass(
  ...
  var halfWinLength = Math.ceil(win.length / 2)
