...
template<typename Sample> inline Sample cutoffToP(Sample sampleRate, Sample cutoffHz) {
  constexpr Sample twopi = 2 * Sample(3.1415926535897932384626433832795028);

  auto omega_c = twopi * cutoffHz / sampleRate;
  auto y = Sample(1) - std::cos(omega_c);
  return std::sqrt((y + Sample(2)) * y) - y;
}
...
public:
  ...
  // `processPeakHold` and `applyGain` can be used to apply characteristic curve in
  // JavaScript.
  Sample processPeakHold(Sample inAbs) { return peakhold.process(inAbs); }

  Sample applyGain(Sample input, Sample peakAmp, Sample candidate) {
    auto released = processRelease(candidate);
    auto gainAmp = std::min(released, candidate);
    auto targetAmp = peakAmp < gateAmp ? 0 : gainAmp;
    auto smoothed = smoother.process(targetAmp);
...
EMSCRIPTEN_BINDINGS(UhhyouBasicLimiter) {
  function("add", &add<double>);

  class_<PeakHold<double>>("PeakHold")
    .constructor()
    .function("resize", &PeakHold<double>::resize)
    .function("reset", &PeakHold<double>::reset)
    .function("setFrames", &PeakHold<double>::setFrames)
    .function("process", &PeakHold<double>::process);

  class_<DoubleAverageFilter<double>>("DoubleAverageFilter")
    .constructor()
    .function("resize", &DoubleAverageFilter<double>::resize)
    .function("reset", &DoubleAverageFilter<double>::reset)
    .function("setFrames", &DoubleAverageFilter<double>::setFrames)
    .function("process", &DoubleAverageFilter<double>::process);

  class_<Limiter<double>>("Limiter")
    .constructor()
    .function("latency", &Limiter<double>::latency)
    .function("resize", &Limiter<double>::resize)
    .function("reset", &Limiter<double>::reset)
    .function("prepare", &Limiter<double>::prepare)
    .function("process", &Limiter<double>::process)
    .function("processPeakHold", &Limiter<double>::processPeakHold)
    .function("applyGain", &Limiter<double>::applyGain);
}
