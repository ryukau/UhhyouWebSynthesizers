...
function randomize() {
  for (const key in param) {
    ...
    if (key === "formantTracking") {
      // param[key].dsp = util.uniformDistributionMap(Math.random(), 0, 0.5);
      continue;
    }
...
const scales = {
  ...
  chordPitch: new parameter.DecibelScale(util.ampToDB(1), util.ampToDB(8), false),
...
const param = {
  ...
  highShelfHz: new parameter.Parameter(8000, scales.highShelfHz, true),
