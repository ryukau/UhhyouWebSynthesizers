...
Generators in this collection output WAVE file. These files are intended to be used in external DAWs or samplers. For real-time effects and synthesizers, take a look at [my plugin repository](https://github.com/ryukau/VSTPlugins).
...
# Offline Use
To use UhhyouWebSynthesizers without the internet:

1. Install [Python 3](https://www.python.org/).
2. Download and extract (unzip) this repository. The extracted directory is referred as `UhhyouWebSynthesizers` in the following commands.
3. Open terminal (PowerShell on Windows), and run following commands.
...
# License
Apache-2.0 except `lib` and `oldsynth/PADchoir` directory. `LICENSE.txt` only contains the Apache-2.0 license text.

`lib` directory contains the licenses for each library.

`oldsynth/PADchoir` directory follows GPLv2+. `oldsynth/PADchoir/oscilgen.js` is a JavaScript port of `OscilGen.cpp` in [Yoshimi](https://yoshimi.github.io/downloads.html), which is under GPLv2+.
